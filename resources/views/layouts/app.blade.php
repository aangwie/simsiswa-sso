...
<body class="h-full antialiased font-sans flex flex-col md:flex-row overflow-hidden bg-slate-50" x-data="{ 
        sidebarOpen: false, 
    ...
    <!-- Sidebar -->
    <aside
        class="fixed inset-y-0 left-0 z-50 bg-indigo-900 text-white transform transition-all duration-300 ease-in-out md:relative md:translate-x-0 flex flex-col shadow-2xl"
        :class="{
        ...
        <div
            class="flex items-center justify-between h-20 px-6 bg-indigo-950 border-b border-indigo-800/50 overflow-hidden">
            <div class="flex items-center gap-3 min-w-max">
                @if($websiteLogo)
                    <div
                        class="w-10 h-10 flex-shrink-0 rounded-xl bg-white p-1 flex items-center justify-center shadow-lg shadow-indigo-500/30">
                        <img src="{{ $websiteLogo }}" alt="Logo" class="w-full h-full object-contain">
                    </div>
                @else
                    <div
                        class="w-10 h-10 flex-shrink-0 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center shadow-lg shadow-indigo-500/30">
                ...
                <div x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                    ...
                    <h1
                        class="text-xl font-bold tracking-wider relative top-px text-transparent bg-clip-text bg-gradient-to-r from-white to-indigo-200">
                        {{ $websiteName }}
                    </h1>
                </div>
            </div>
            ...
            <div class="flex gap-2">
                ...
                <button @click="sidebarMinimized = !sidebarMinimized"
                    class="hidden md:flex p-1.5 rounded-lg bg-indigo-800/50 text-indigo-300 hover:text-white transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 transform transition-transform"
                        :class="{'rotate-180': sidebarMinimized}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M11 19l-7-7 7-7m8 14l-7-7 7-7" />
                    </svg>
                </button>
                <button @click="sidebarOpen = false"
                    class="md:hidden text-indigo-200 hover:text-white transition-colors">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar overflow-x-hidden">
            <p x-show="!sidebarMinimized"
                class="px-3 text-xs font-semibold text-indigo-300 uppercase tracking-wider mb-2">Main Menu</p>

            <a href="{{ route('dashboard') }}"
                class="group flex items-center px-3 py-3 text-sm font-medium rounded-xl border border-transparent 
            ...
            <div class="space-y-1">
                <button @click="manajemenOpen = !manajemenOpen"
                    ...
                    :class="{'bg-indigo-800/30': manajemenOpen}" :title="sidebarMinimized ? 'Manajemen' : ''">
                    <div class="flex items-center">
                        ...
                        <span x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 -translate-x-2"
                            x-transition:enter-end="opacity-100 translate-x-0">Manajemen Siswa</span>
                    </div>
                    <svg x-show="!sidebarMinimized" class="h-4 w-4 transform transition-transform"
                        :class="{'rotate-180': manajemenOpen}" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>

                <div x-show="manajemenOpen && !sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-y-2"
                    x-transition:enter-end="opacity-100 translate-y-0" class="pl-11 pr-3 space-y-1">
                    <a href="{{ route('students.index') }}"
                        class="block py-2 text-sm {{ request()->routeIs('students.*') ? 'text-white font-bold' : 'text-indigo-300 hover:text-white' }} transition-colors">Data
                        Siswa</a>
                    <a href="{{ route('classes.index') }}"
                        class="block py-2 text-sm {{ request()->routeIs('classes.*') ? 'text-white font-bold' : 'text-indigo-300 hover:text-white' }} transition-colors">Data
                        Kelas</a>
                    <a href="{{ route('subjects.index') }}"
                        class="block py-2 text-sm {{ request()->routeIs('subjects.*') ? 'text-white font-bold' : 'text-indigo-300 hover:text-white' }} transition-colors">Mata
                        Pelajaran</a>
                </div>

                <!-- Floating Tooltip for Minimized Sidebar -->
                <div x-show="manajemenOpen && sidebarMinimized"
                    class="absolute left-20 py-2 w-48 bg-indigo-900 rounded-xl shadow-xl border border-indigo-700 z-50">
                    <a href="{{ route('students.index') }}"
                        class="block px-4 py-2 text-sm text-indigo-200 hover:bg-indigo-800 hover:text-white">Data
                        Siswa</a>
                    <a href="{{ route('classes.index') }}"
                        class="block px-4 py-2 text-sm text-indigo-200 hover:bg-indigo-800 hover:text-white">Data
                        Kelas</a>
                    <a href="{{ route('subjects.index') }}"
                        class="block px-4 py-2 text-sm text-indigo-200 hover:bg-indigo-800 hover:text-white">Mata
                        Pelajaran</a>
                </div>
            </div>

            <!-- Manajemen Rapor Submenu -->
            <div class="space-y-1">
                <button @click="raporOpen = !raporOpen"
                    class="w-full group flex items-center justify-between px-3 py-3 text-sm font-medium rounded-xl border border-transparent text-indigo-200 hover:bg-indigo-800/40 hover:text-white hover:border-indigo-700/30 transition-all"
                    :class="{'bg-indigo-800/30': raporOpen}" :title="sidebarMinimized ? 'Manajemen Rapor' : ''">
                    <div class="flex items-center">
                        <svg class="h-5 w-5 flex-shrink-0 text-indigo-400 group-hover:text-indigo-300 transition-colors"
                            :class="{'mr-3': !sidebarMinimized}" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <span x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="opacity-0 -translate-x-2"
                            x-transition:enter-end="opacity-100 translate-x-0">Manajemen Nilai</span>
                    </div>
                    <svg x-show="!sidebarMinimized" class="h-4 w-4 transform transition-transform"
                        :class="{'rotate-180': raporOpen}" xmlns="http://www.w3.org/2000/svg" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                    </svg>
                </button>

                <div x-show="raporOpen && !sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-y-2"
                    x-transition:enter-end="opacity-100 translate-y-0" class="pl-11 pr-3 space-y-1">
                    <a href="{{ route('semesters.index') }}"
                        class="block py-2 text-sm {{ request()->routeIs('semesters.*') ? 'text-white font-bold' : 'text-indigo-300 hover:text-white' }} transition-colors">Semester</a>
                ...
                <!-- Floating Tooltip for Minimized Sidebar -->
                <div x-show="raporOpen && sidebarMinimized"
                    class="absolute left-20 py-2 w-48 bg-indigo-900 rounded-xl shadow-xl border border-indigo-700 z-50">
                    <a href="{{ route('semesters.index') }}"
                        class="block px-4 py-2 text-sm text-indigo-200 hover:bg-indigo-800 hover:text-white">Semester</a>
                    <a href="{{ route('reports.index') }}"
            ...
            @if(auth()->user()->role !== 'teacher')
                <p x-show="!sidebarMinimized"
                    class="px-3 text-xs font-semibold text-indigo-300 uppercase tracking-wider mt-4 mb-2">Pengaturan</p>
                <a href="{{ route('settings.website') }}"
                    class="group flex items-center px-3 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('settings.website') ? 'bg-indigo-800/60 text-white border-indigo-700/50 shadow-inner' : 'text-indigo-200 border border-transparent hover:bg-indigo-800/40 hover:text-white hover:border-indigo-700/30 transition-all' }}"
                    ...
                    <span x-show="!sidebarMinimized" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 -translate-x-2"
                        x-transition:enter-end="opacity-100 translate-x-0">Pengaturan Website</span>
                </a>

                <a href="{{ route('settings.index') }}"
                    class="group flex items-center px-3 py-3 text-sm font-medium rounded-xl {{ request()->routeIs('settings.index') ? 'bg-indigo-800/60 text-white border-indigo-700/50 shadow-inner' : 'text-indigo-200 border border-transparent hover:bg-indigo-800/40 hover:text-white hover:border-indigo-700/30 transition-all' }}"
        ...
        <div class="px-4 py-5 bg-indigo-950/50 border-t border-indigo-800/50"
            ...
            <div class="flex items-center gap-3 mb-4" :class="{'justify-center': sidebarMinimized}">
                <div class="h-10 w-10 flex-shrink-0 rounded-full bg-gradient-to-r from-blue-400 to-indigo-500 p-0.5">
                    <div
                        class="w-full h-full rounded-full bg-indigo-900 flex items-center justify-center border-2 border-indigo-950 text-white font-bold text-sm">
            ...
            <form action="{{ route('logout') }}" method="POST" id="logout-form">
                ...
                <button type="button" onclick="confirmLogout()"
                    class="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 hover:border-red-500/40 rounded-lg transition-colors"
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-slate-50/50">
            ...
            <div class="flex items-center justify-between h-20 px-6">
                ...
                <div class="flex items-center gap-4">
                    <button @click="sidebarOpen = true"
                        class="md:hidden p-2 rounded-xl text-slate-500 hover:bg-slate-100 hover:text-slate-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center gap-4">
                    <div class="relative" x-data="{ userMenuOpen: false }">
                        <button @click="userMenuOpen = !userMenuOpen" @click.away="userMenuOpen = false"
                            class="flex items-center gap-3 p-1 rounded-full hover:bg-slate-100 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                            <span
                                class="text-sm font-medium text-slate-700 hidden sm:block pl-2">{{ auth()->user()->name ?? 'User Name' }}</span>
                            <div
                                class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center shadow-sm">
                                <span
                                    class="text-indigo-700 font-bold text-sm">{{ substr(auth()->user()->name ?? 'U', 0, 1) }}</span>
                            </div>
                        </button>
                        ...
                        <div x-show="userMenuOpen" x-transition:enter="transition ease-out duration-100"
                            ...
                            <div class="py-1">
                                <a href="#"
                                    class="block px-4 py-2 text-sm text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition-colors">Profile
                                    Settings</a>
                            </div>
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto relative">
            <div
                class="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-[0.03] z-0 pointer-events-none">
            </div>
            ...
            <footer class="relative z-10 border-t border-slate-200 bg-white shadow-sm mt-auto">
                <div class="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <div
                        class="flex items-center gap-4 text-xs font-medium text-slate-400 bg-slate-50 px-4 py-2 rounded-lg border border-slate-100">
