...
<head>
    ...
    <style>
        ...
        .animate-fadeInUp-delay { animation: fadeInUp 0.8s ease-out 0.3s forwards; opacity: 0; }
        .animate-fadeInUp-delay2 { animation: fadeInUp 0.8s ease-out 0.5s forwards; opacity: 0; }
        .animate-fadeInScale { animation: fadeInScale 0.6s ease-out 0.2s forwards; opacity: 0; }
        .shimmer {
...
<body class="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex flex-col relative overflow-x-hidden text-white">
    ...
    <div class="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        @if($isLulus)
        <div class="absolute top-1/4 left-1/4 w-[300px] h-[300px] rounded-full bg-emerald-500/10 blur-[100px] animate-float2"></div>
        @endif
    </div>
    ...
    <div class="flex-1 flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 relative z-10 py-8 -mt-10">
        ...
        <div class="animate-fadeInScale mb-6">
            @if($isLulus)
                <div class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 font-bold text-lg shadow-lg shadow-emerald-500/10">
            ...
            @else
                <div class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-red-500/20 border border-red-500/30 text-red-300 font-bold text-lg shadow-lg shadow-red-500/10">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="w-full max-w-lg animate-fadeInUp">
            <div class="bg-white/[0.07] backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl shadow-black/20 overflow-hidden">

                <!-- Card Header -->
                <div class="px-8 pt-8 pb-4 text-center">
                    <h1 class="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-white to-indigo-200">
                ...
                <div class="px-8 pb-6">
                    <div class="bg-white/5 rounded-2xl border border-white/10 p-6 space-y-4">
                        ...
                        <div class="flex items-center justify-between border-b border-white/5 pb-3">
                            ...
                            <span class="text-sm font-semibold text-indigo-200">{{ $student->schoolClass->name ?? '-' }}</span>
                        </div>
                        <div class="flex items-center justify-between border-b border-white/5 pb-3">
                            <span class="text-sm text-indigo-300/60 font-medium">Rata-Rata Nilai</span>
                            <span class="text-sm font-bold {{ $isLulus ? 'text-emerald-400' : 'text-red-400' }}">{{ number_format($average, 2) }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-indigo-300/60 font-medium">Status</span>
                            @if($isLulus)
                                <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-xs font-bold border border-emerald-500/30">
                            ...
                            @else
                                <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-red-500/20 text-red-300 text-xs font-bold border border-red-500/30">
                ...
                <div class="px-8 pb-8 space-y-3">
                    ...
                    <a href="{{ route('cek-kelulusan') }}" id="btn-cek-ulang" class="w-full flex justify-center items-center gap-2.5 py-3.5 px-4 rounded-2xl text-sm font-semibold text-indigo-200 bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-300">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
