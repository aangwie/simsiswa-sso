...
@section('title', 'Entry Nilai Akhir - SIMSiswa')
@section('header', 'Entry Nilai Akhir')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row items-center justify-between gap-4">
        <div class="text-center md:absolute md:left-1/2 md:-translate-x-1/2 w-full md:w-auto">
            <h2 class="text-lg font-bold text-slate-800">
                Entry Nilai Akhir Kelas {{ $class->name }}
            </h2>
        </div>
        
        <div class="flex items-center gap-2 w-full md:w-auto justify-end">
            <a href="{{ route('nilai-akhir.export-excel', ['class' => $class->id]) }}" class="inline-flex items-center justify-center gap-2 px-2 md:px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                <span class="hidden md:inline">Ekspor Nilai Akhir</span>
            </a>
            <button type="button" onclick="showImportModal()" class="inline-flex items-center justify-center gap-2 px-2 md:px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 hover:text-indigo-600 rounded-xl transition-all shadow-sm">
                ...
                <span class="hidden md:inline">Import Nilai Akhir</span>
            </button>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <form action="{{ route('nilai-akhir.store', $class->id) }}" method="POST">
            @csrf
                                ...
                                @foreach($subjects as $subject)
                                    @php
                                        ...
                                        if ($gradeValue !== '' && $gradeValue !== null) {
                                            $totalGrade += floatval($gradeValue);
                                            $countGrade++;
                                        }
                                    @endphp
...
<div id="importModal" class="fixed inset-0 z-50 flex items-center justify-center hidden" role="dialog" aria-modal="true">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 overflow-hidden transform transition-all">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 class="text-lg font-bold text-slate-800">Import Data Nilai Akhir</h3>
        ...
        <form action="{{ route('nilai-akhir.import-excel', $class->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="p-6 space-y-4">
                <div class="bg-indigo-50 text-indigo-700 p-4 rounded-xl text-sm border border-indigo-100">
                    ...
                    <ul class="list-disc list-inside space-y-1">
                        <li>Unduh template Nilai Akhir untuk kelas <strong>{{ $class->name }}</strong>.</li>
                        <li>Isi Nilai Akhir pada kolom setiap pelajaran.</li>
                        <li>Format file harus <strong>.xlsx</strong>.</li>
                        <li>Jangan mengubah susunan kolom ID Siswa & Nama.</li>
                    </ul>
                    <div class="mt-3">
                        <a href="{{ route('nilai-akhir.import-template', $class->id) }}" class="inline-flex items-center gap-1.5 text-indigo-600 hover:text-indigo-800 font-medium bg-white px-3 py-1.5 rounded-lg border border-indigo-200 shadow-sm transition-colors">
