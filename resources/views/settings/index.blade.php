            ...
            <form action="{{ route('settings.update') }}" method="POST" class="p-6 space-y-8">
                ...
                <div class="space-y-4">
                    <h3 class="text-sm font-bold text-slate-400 uppercase tracking-widest border-b border-slate-50 pb-2">
                        Konfigurasi GitHub</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1 md:col-span-2">
                            <label class="block text-xs font-semibold text-slate-700 tracking-wide uppercase">GitHub Access
                                Token</label>
                            <input type="password" name="github_token"
                                value="{{ $settings->where('key', 'github_token')->first()?->value }}"
                                placeholder="ghp_xxxxxxxxxxxx"
                                class="w-full rounded-xl border-slate-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 px-4 py-2 text-sm font-mono">
                            <p class="text-[10px] text-slate-400">Personal Access Token dengan izin read-only ke repository
                                ini. Repository URL dan Branch akan dideteksi secara otomatis dari sistem.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-slate-100 flex justify-end">
                    <button type="submit"
                        class="px-8 py-2.5 bg-gradient-to-r from-purple-500 to-blue-500 text-white font-bold rounded-xl hover:from-purple-600 hover:to-blue-600 transition-all shadow-lg shadow-indigo-100 uppercase text-xs tracking-widest">
                        Simpan Perubahan
                    </button>
                </div>
            </form>
        ...
        <div class="bg-slate-900 rounded-2xl shadow-xl border border-slate-800 overflow-hidden"
            ...
            <div class="p-6 border-b border-slate-800 flex items-center justify-between">
                ...
                <button @click="updateApp()" :disabled="updating"
                    ...
                    <template x-if="updating">
                        <span class="flex items-center gap-2">
                            <svg class="animate-spin h-4 w-4 text-slate-900" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24">
